...
function App() {
  ...
  const handleUpload = async () => {
    ...
    const res = await fetch("http://127.0.0.1:8000/upload/post-file", {
      method: "POST",
      body: formData,
    });
    ...
    setMessages([
      {
        role: "assistant",
        content: `"${file.name}" uploaded! Ask me anything.`,
      },
    ]);
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <div className="flex items-center gap-3 px-5 py-4 bg-white border-b border-gray-200">
        <div className="w-8 h-8 rounded-lg bg-indigo-600 flex items-center justify-center text-white font-medium text-sm">
          D
        </div>
        <div>
      ...
      <div className="flex items-center gap-3 px-4 py-2 bg-gray-100 border-b border-gray-200">
        ...
        <button
          ...
          disabled={!file}
          className="text-sm px-4 py-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-40 cursor-pointer"
        >
          Upload
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-3">
        ...
        {messages.map((msg, i) => (
          ...
          >
            <div
              className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium flex-shrink-0 ${msg.role === "user" ? "bg-indigo-100 text-indigo-700" : "bg-emerald-100 text-emerald-700"}`}
            >
              {msg.role === "user" ? "U" : "AI"}
            </div>
            <div
              className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${msg.role === "user" ? "bg-indigo-600 text-white rounded-br-sm" : "bg-white border border-gray-200 text-gray-800 rounded-bl-sm"}`}
            >
              {msg.content}
            </div>
          </div>
      ...
      <div className="flex items-center gap-2 px-4 py-3 bg-white border-t border-gray-200">
        ...
        <button
          onClick={handleAsk}
          disabled={!input.trim() || !collection || loading}
